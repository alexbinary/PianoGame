...
class CountingExerciseTimingScene: SKScene {
    ...
    var currentQuestionDuration: TimeInterval = 10
    ...
    var currentQuestionTerminated = false
    ...
    var timerProgressBarNode: SKShapeNode! = nil
    let timerProgressBarNodeScaleActionKey = "timer"
    
    
    var activeNotes: Set<NoteCode> = []
    ...
    func initStaticUI() {
        ...
        self.timerProgressBarNode = SKShapeNode(rect: CGRect(x: 0, y: 0, width: self.size.width, height: 10))
        self.timerProgressBarNode.fillColor = colorPalette.foregroundColor
        self.timerProgressBarNode.position = CGPoint(x: -self.size.width/2.0, y: -self.size.height/2.0)
        self.addChild(self.timerProgressBarNode)
    }
    ...
    func loadNextQuestion() {
        ...
        self.currentQuestionTerminated = false
    }
    
    
    func startQuestionTimer() {
        
        self.timerProgressBarNode.xScale = 0
        
        let scaleAction = SKAction.scaleX(to: 1, duration: self.currentQuestionDuration)
        let completionAction = SKAction.run {
            
            self.currentQuestionTerminated = true
        ...
        self.timerProgressBarNode.run(SKAction.sequence([scaleAction, completionAction]), withKey: self.timerProgressBarNodeScaleActionKey)
    }
    ...
    func onNoteOn(noteCode: NoteCode, velocity: Velocity) {
        ...
        self.activeNotes.insert(noteCode)
    ...
    func onNoteOff(noteCode: NoteCode) {
        ...
        if self.currentQuestionTerminated, self.activeNotes.isEmpty {
